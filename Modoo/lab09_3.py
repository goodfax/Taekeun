...
class wide_deep_nn():
    def __init__(self, nb_classes):
        ...
        self.b4 = tf.Variable(tf.random.normal((nb_classes,)), name='bias4')

        self.variable = [self.W1, self.b1, self.W2, self.b2,
    ...
    def accuracy_fn(selfself, hypothesis, labels):
    ...
    def grad(self, hypothesis, features, labels):
        ...
        return tape.gradient(loss_value, self.variable)
    ...
    def fit(self, dataset, EPOCHS=20000, verbose=500):
        ...
        for step in range(EPOCHS):
            for features,labels in dataset:
                ...
                optimizer.apply_gradients(grads_and_vars=zip(grads, self.variable))
                if step % verbose == 0:
                    print("Step: {}, Loss: {:.4f}".format(step,
                                                          self.loss_fn(self.deep_nn(features),
                                                                       features, labels)))
