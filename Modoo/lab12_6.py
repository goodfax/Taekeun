...
checkpoint_prefix = os.path.join(checkpoint_dir, 'seq2seq\seq2seq')
checkponit = tf.train.Checkpoint(optimizer=optimizer, encoder=encoder, decoder=decoder)

summary_prefix = os.path.join(checkpoint_dir, 'summary')
summary_writer = tf.summary.create_file_writer(logdir=checkpoint_prefix)
...
for epoch in range(epochs):
    ...
    if epoch % 10 == 0:
        ...
        checkponit.save(file_prefix=checkpoint_prefix)


checkponit.restore(tf.train.latest_checkpoint(checkpoint_dir))
